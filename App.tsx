...
const App: React.FC = () => {
  ...
  const loadDatabase = async () => {
    ...
    try {
      ...
      setUnreadCount(storedAgreements.filter(a => a.status === 'submitted').length);
      setStaffConfig(storedStaff);

      if (storedDebtors.length > 0) {
        setDebtors(storedDebtors);
      } else {
  ...
  const handleClientSubmit = async (data: AgreementData) => {
    ...
    setUnreadCount(updated.filter(a => a.status === 'submitted').length);
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans bg-slate-50 text-slate-900">
      ...
      <main className="flex-grow">
        {view === 'CLIENT_PORTAL' && (
          <AgreementForm debtors={debtors} onSubmit={handleClientSubmit} />
        )}
      ...
      <footer className="py-6 border-t bg-white text-center">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">© 2024 Kenya Dairy Board | Regulatory Payment Compliance</p>
      </footer>
    </div>
  );
};
