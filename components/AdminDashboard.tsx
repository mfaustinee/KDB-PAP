...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ agreements, debtors, staffConfig, onAction, onDebtorUpdate, onStaffUpdate }) => {
  ...
  const exportToCSV = () => {
    if (debtors.length === 0) return alert("No data to export.");
    const headers = ["DBO Name", "Permit No", "County", "Total Arrears", "Period", "Tel"];
    ...
    link.setAttribute("download", `KDB_Ledger_Export_${new Date().toISOString().split('T')[0]}.csv`);
              ...
              agreements.map(a => (
                <button key={a.id} onClick={() => { setSelectedReviewId(a.id); setIsRejecting(false); }} className={`w-full p-6 rounded-[32px] border text-left transition-all ${selectedReviewId === a.id ? 'border-emerald-600 bg-emerald-50/50 shadow-xl' : 'border-white bg-white hover:border-emerald-200 shadow-sm'}`}>
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className={`w-2 h-2 rounded-full ${a.status === 'submitted' ? 'bg-amber-400' : a.status === 'approved' ? 'bg-emerald-500' : 'bg-rose-500'}`}></div>
                  </div>
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-white p-6 rounded-[32px] shadow-sm border border-slate-100">
                ...
                <div className="flex gap-4 w-full sm:w-auto">
                  <button onClick={exportToCSV} className="flex-1 sm:flex-none px-6 py-4 bg-emerald-50 text-emerald-700 rounded-2xl font-black text-[11px] uppercase tracking-widest flex items-center justify-center hover:bg-emerald-100 transition-all">
